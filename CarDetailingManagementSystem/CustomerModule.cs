...
namespace CarDetailingManagementSystem
{
    public partial class CustomerModule : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (check)
                {
                    ...
                    {
                        cm = new SqlCommand("INSERT INTO tbCustomer (VehicleId, CustomerName, CustomerPhone, CustomerCarNumber, CustomerCarModel," +
                            "               CustomerAddress, CustomerPoints) VALUES (@vid, @name, @phone, @carno, @carmodel, @address, @points)", dbcon.connect());

                        cm.Parameters.AddWithValue("@vid", cbCarType.SelectedValue); // зберігаємо id типу автомобіля
                        ...
                        cm.Parameters.AddWithValue("@points", Convert.ToInt32(udPoints.Text));
            ...
            try
            {
                ...
                if (check)
                {
                    if (MessageBox.Show("Are you sure you want to edit this Customer?", "Customer Editing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        cm = new SqlCommand(
                            "UPDATE tbCustomer " +
                            "SET VehicleId = @vid, CustomerName = @name, CustomerPhone = @phone, " +
                            "CustomerCarNumber = @carno, CustomerCarModel = @carmodel, " +
                            "CustomerAddress = @address, CustomerPoints = @points " +
                            "WHERE CustomerId = @id", dbcon.connect());
        ...
        private void CustomerModule_Load(object sender, EventArgs e)
        {
            ...
            cbCarType.DisplayMember = "VehicleName";
            cbCarType.ValueMember = "VehicleId";
            if (vid > 0)
