    ...
    public partial class Employee : Form
    {
        ...
        private void dgvEmployer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete") // if you want to delete the record to click the delete icon on the datagridview
            {
                try
                {
                    ...
                    {
                        cm = new SqlCommand("DELETE FROM tbEmployee WHERE EmployeeId LIKE'" + dgvEmployer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", dbcon.connect());
                        dbcon.open();
                        cm.ExecuteNonQuery();
                        dbcon.close();
        ...
        public void loadEmployer()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM tbEmployee WHERE CONCAT (EmployeeName,EmployeeAddress,EmployeeRole) LIKE '%" + txtSearch.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
