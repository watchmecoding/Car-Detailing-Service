...
namespace CarDetailingManagementSystem
{
    public partial class EmployeeModule : Form
    {
        ...
        bool check = false;
        Employee employer;
        public EmployeeModule(Employee emp)
        {
            InitializeComponent();
            employer = emp;
        ...
        {//type try and then double press Tab key
            try
            {
                ...
                if (check)
                {
                    ...
                    {
                        cm = new SqlCommand("INSERT INTO tbEmployee(EmployeeName,EmployeePhone,EmployeeAddress,EmployeeDOB,EmployeeGender,EmployeeRole,EmployeeSalary,EmployeePassword, CompanyId)VALUES(@EmployeeName,@EmployeePhone,@EmployeeAddress,@EmployeeDOB,@EmployeeGender,@EmployeeRole,@EmployeeSalary,@EmployeePassword,@CompanyId)", dbcon.connect());
                        cm.Parameters.AddWithValue("@EmployeeName", txtName.Text);
                        cm.Parameters.AddWithValue("@EmployeePhone", txtPhone.Text);
                        cm.Parameters.AddWithValue("@EmployeeAddress", txtAddress.Text);
                        cm.Parameters.AddWithValue("@EmployeeDOB", dtDob.Value);
                        cm.Parameters.AddWithValue("@EmployeeGender", rdMale.Checked ? "Male" : "Female");//like if condition
                        cm.Parameters.AddWithValue("@EmployeeRole", cbRole.Text);
                        cm.Parameters.AddWithValue("@EmployeeSalary", txtSalary.Text);
                        cm.Parameters.AddWithValue("@EmployeePassword", txtPassword.Text);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (check)
                {
                    ...
                    {
                        cm = new SqlCommand("UPDATE tbEmployee SET EmployeeName=@name, EmployeePhone=@phone, EmployeeAddress=@address, EmployeeDOB=@dob, EmployeeGender=@gender, EmployeeRole=@role, EmployeeSalary=@salary, EmployeePassword=@password WHERE EmployeeId=@id", dbcon.connect());
