...
namespace CarDetailingManagementSystem
{
    public partial class SettlePayment : Form
    {
        ...
        CashReg cash;
        public SettlePayment(CashReg cashform)
        {
            InitializeComponent();
            cash = cashform;
        }
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        dbcon.executeQuery("UPDATE tbCashReg SET CashStatus='Sold', Price='" + cash.dgvCash.Rows[i].Cells[9].Value.ToString() + "' WHERE CashId='" + cash.dgvCash.Rows[i].Cells[1].Value.ToString() + "'");
                        dbcon.executeQuery("UPDATE tbCustomer SET CustomerPoints += " + 1 + " WHERE CustomerId='" + cash.customerId + "'");
                    }
