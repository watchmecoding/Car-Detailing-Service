    ...
    public partial class Report : Form
    {
        ...
        public void loadTopSelling()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT TOP 10 se.ServiceName,count(ca.ServiceId) AS qty, ISNULL(SUM(ca.Price),0) AS total FROM tbCashReg AS ca JOIN tbService AS se ON ca.ServiceId=se.ServiceId WHERE ca.CashDate BETWEEN '" + dtFromTopSelling.Value.ToString() + "' AND '" + dtToTopSelling.Value.ToString() + "' AND CashStatus LIKE 'Sold' " +
                    "GROUP BY se.ServiceName ORDER BY qty DESC", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        public void loadRevenus()
        {
            ...
            try
            {
                ...
                double total = 0;
                cm = new SqlCommand("SELECT CashDate,ISNULL(sum(Price),0) AS total FROM tbCashReg WHERE CashDate BETWEEN '" + dtFromRevenud.Value.ToString() + "' " +
                    "AND '" + dtToRevenus.Value.ToString() + "' AND CashStatus LIKE 'Sold' GROUP BY CashDate", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        public void loadCostofGood()
        {
            try
            {
                ...
                double total = 0;
                cm = new SqlCommand("SELECT ExpenseName,ExpenseCost,ExpenseDate FROM tbExpense WHERE ExpenseDate BETWEEN '" + dtFromCoG.Value.ToString() + "' AND '" + dtToCoG.Value.ToString() + "'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        {
            txtRevenus.Text = extractData("SELECT ISNULL(SUM(Price),0) AS total FROM tbCashReg WHERE CashDate BETWEEN '" + dtFromGP.Value.ToString() + "' AND '" + dtToGP.Value.ToString() + "'").ToString("#,##0.00");
            txtCoG.Text = extractData("SELECT ISNULL(SUM(ExpenseCost),0) AS cost FROM tbExpense WHERE ExpenseDate BETWEEN '" + dtFromGP.Value.ToString() + "' AND '" + dtToGP.Value.ToString() + "'").ToString("#,##0.00");
