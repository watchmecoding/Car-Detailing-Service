    ...
    public partial class Customer : Form
    {
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete") // if you want to delete the record to click the delete icon on the datagridview
            {
                try
                {
                    ...
                    {
                        cm = new SqlCommand("DELETE FROM tbCustomer WHERE CustomerId LIKE'" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", dbcon.connect());
                        dbcon.open();
                        cm.ExecuteNonQuery();
                        dbcon.close();
        ...
        public void loadCustomer()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT CustomerId, CustomerName, CustomerPhone, CustomerCarNumber, CustomerCarModel, V.VehicleName, CustomerAddress, CustomerPoints FROM tbCustomer AS C INNER JOIN tbVehicleType AS V ON C.VehicleId=V.VehicleId WHERE CONCAT (CustomerName,CustomerCarNumber,CustomerCarModel,CustomerAddress) LIKE '%" + txtSearch.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        {
            int i = 0;
            cm = new SqlCommand("SELECT VehicleId FROM tbVehicleType WHERE VehicleName LIKE '" + str + "' ", dbcon.connect());
            dbcon.open();
            dr = cm.ExecuteReader();
            dr.Read();
            if (dr.HasRows)
            {
                i = int.Parse(dr["VehicleId"].ToString());
            }
            dbcon.close();
            return i;
        }
